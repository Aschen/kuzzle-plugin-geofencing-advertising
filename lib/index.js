...
class CorePlugin {
  ...
  async geofenceRegister (request) {
    ...
    const bounding_box = request.input.args.bounding_box

    const polygons = generatePolygons(count, bounding_box)
  ...
  async geofenceTest (request) {
    ...
    ids.forEach(id => {
      this._getDocument(id).then(document => documents.push(document));
    });
